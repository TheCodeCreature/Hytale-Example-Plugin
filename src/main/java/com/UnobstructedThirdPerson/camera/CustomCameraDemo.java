...
/**
 * Custom camera demo that allows setting custom pitch and yaw rotation values.
 * Extends the base CameraDemo functionality but with configurable camera rotation.
 */
public class CustomCameraDemo {
    ...
    /**
     * Create a custom camera demo with specified rotation.
     * 
     ...
     */
    public CustomCameraDemo(float yaw, float pitch) {
        this.yaw = yaw;
        this.pitch = pitch;
        this.eventRegistry = new EventRegistry(
            new CopyOnWriteArrayList<>(), 
            () -> this.isActive, 
            "CustomCameraDemo is not active!", 
            HytaleServer.get().getEventBus()
        );
        this.cameraSettings = createServerCameraSettings();
    }
                    ...
                    if (!lastTargetBlock.equals(targetBlock)) {
                        BlockIterator.iterateFromTo(
                            ...
                            (xx, y, zx, px, py, pz, qx, qy, qz) -> {
                                world.getChunk(ChunkUtil.indexChunkFromBlock(xx, zx)).setBlock(xx, y, zx, blockIdx);
                                return true;
                            }
                        );
                    } else {
    ...
    @Nonnull
    private ServerCameraSettings createServerCameraSettings() {
        ...
        cameraSettings.rotation = new Direction(this.yaw, this.pitch, 0.0F);
