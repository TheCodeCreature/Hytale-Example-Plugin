...
class ResourceTypeResolverTest {
    ...
    // ────────────────────────────────────────────────────────────
    @Nested
    class IsResourceTypeExclusivelyNatural {

        @Test
        void returnsFalseWhenMixOfNaturalAndNonNatural() {
            var items = new LinkedHashMap<String, Item>();
            items.put("Wood_Log_Oak", logOak());
            items.put("Wood_Hardwood_Planks", planks());
            installMinimal(items);

            assertFalse(ResourceTypeResolver.isResourceTypeExclusivelyNatural(
                    "Wood_Hardwood", Set.of("Wood_Log_Oak")),
                    "Wood_Hardwood matches both natural log and non-natural planks");
        }

        @Test
        void returnsTrueWhenAllItemsAreNatural() {
            var items = new LinkedHashMap<String, Item>();
            items.put("Wood_Log_Oak", logOak());
            items.put("Wood_Log_Birch", logBirch());
            installMinimal(items);

            assertTrue(ResourceTypeResolver.isResourceTypeExclusivelyNatural(
                    "Wood_Trunk", Set.of("Wood_Log_Oak", "Wood_Log_Birch")),
                    "Wood_Trunk matches only natural trunk/log items");
        }

        @Test
        void returnsFalseWhenNoItemsDeclareResourceType() {
            var items = new LinkedHashMap<String, Item>();
            items.put("Wood_Hardwood_Planks", planks());
            installMinimal(items);

            assertFalse(ResourceTypeResolver.isResourceTypeExclusivelyNatural(
                    "Stone_Granite", Set.of("Wood_Log_Oak")),
                    "No items declare Stone_Granite — should not return true vacuously");
        }

        @Test
        void returnsFalseWhenSingleNonNaturalItem() {
            var items = new LinkedHashMap<String, Item>();
            items.put("Wood_Hardwood_Planks", planks());
            installMinimal(items);

            assertFalse(ResourceTypeResolver.isResourceTypeExclusivelyNatural(
                    "Wood_Hardwood", Set.of()),
                    "Single non-natural item should return false");
        }

        @Test
        void returnsTrueWhenSingleNaturalItem() {
            var items = new LinkedHashMap<String, Item>();
            items.put("Wood_Log_Oak", logOak());
            installMinimal(items);
            ...
            assertTrue(ResourceTypeResolver.isResourceTypeExclusivelyNatural(
                    "Wood_Hardwood", Set.of("Wood_Log_Oak")),
                    "Single natural item should return true");
        }
    }
}
