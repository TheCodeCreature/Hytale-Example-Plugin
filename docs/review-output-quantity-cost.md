...
| 3 | Anti-pattern | 🔴 **Blocked** | [StencilSelectionPage.java](src/main/java/com/UnobstructedThirdPerson/placeblock/ui/StencilSelectionPage.java#L638) | `isAffordable()` uses `getInputMaterials(recipe, 1)` — same full-cost check. The stencil UI grays out recipes the player can actually afford. Inconsistent with what `PlaceBlockToolInteraction` would charge (both are wrong, but if #1 is fixed and #3 is not, they'd disagree). |
| 4 | Redundancy | 🟡 **Should Fix** | [StencilSelectionPage.java](src/main/java/com/UnobstructedThirdPerson/placeblock/ui/StencilSelectionPage.java#L415-L433) | `updateDetailPanel()` reads `recipe.getInput()` raw and displays full recipe quantities (e.g., "x12 wood"). The UI shows the total recipe cost, not per-unit placement cost. Player sees "12 wood" but should see "6 wood" (per ladder). Not economy-breaking but **misleading UI** that will conflict with the fix. |
...
- **StencilSelectionPage L638** (`isAffordable`) — same replacement
- **StencilSelectionPage L415-433** (`updateDetailPanel`) — replace `recipe.getInput()` raw iteration with `PlaceBlockCostUtil.getPerUnitPlacementCost(recipe)`, then display those quantities
