...
**Scope:** `BlueprintBookParticleLoop`, `BlueprintBookPickStencilInteraction`, `StencilInputListener`, `BoundingBoxRayCast`
...
Package movement is correctly wired — `Plugin.java`, `ParticleCommand.java`, and all test files reference the new `ui.blueprintbook` and `ui.radial` packages with no stale `com.CodeCreature.stencil` imports remaining in live source. However, the compile-restoration skeleton scaffolding (`compileRestorationSkeleton()` methods, `COMPILE_RESTORATION_TODO_ID` constants, and 9 `TODO[S2605261205]` markers) was **not cleaned up** as required by the design doc. More critically, `BlueprintBookParticleLoop` has **missing null guards** on `effectCtrl` and `effect` that will cause NPEs at runtime, and both it and `StencilInputListener` use **deprecated-for-removal Inventory APIs** that will break on the next engine version.
...
graph TB
    subgraph Package Movement
        P1["Plugin.java<br/>import BlueprintBookParticleLoop"]
        P2["Plugin.java<br/>import BlueprintBookPickStencilInteraction"]
        P3["ParticleCommand.java<br/>import BlueprintBookPickStencilInteraction"]
        P4["Test files<br/>correct package"]
    end

    subgraph "Skeleton Debris (Should Remove)"
        S1["BlueprintBookParticleLoop<br/>compileRestorationSkeleton()"]:::orange
        S2["BlueprintBookPickStencilInteraction<br/>compileRestorationSkeleton()"]:::orange
    ...
    subgraph "Null Safety Gaps"
        N1["BlueprintBookParticleLoop<br/>effectCtrl nullable deref L243"]:::red
        N2["BlueprintBookParticleLoop<br/>effect nullable deref L243"]:::red
        N3["BlueprintBookParticleLoop<br/>getHotbar() nullable deref L133"]:::red
    end
    ...
    subgraph "Deprecation Warnings"
        ...
        D4["BlueprintBookParticleLoop<br/>getActiveHotbarSlot() deprecated"]:::orange
        D5["BlueprintBookParticleLoop<br/>getHotbar() deprecated"]:::orange
        D6["BlueprintBookParticleLoop<br/>getCombinedBackpackStorageHotbar() deprecated"]:::orange
    end
...
|---|----------|----------|----------|--------|
| 1 | Anti-pattern | 🔴 Blocked | [BlueprintBookParticleLoop.java](src/main/java/com/CodeCreature/ui/blueprintbook/BlueprintBookParticleLoop.java#L241-L243) | **Missing null guard on `effectCtrl` and `effect`.** `store.getComponent(entityRef, EffectControllerComponent.getComponentType())` returns `@Nullable`. `EntityEffect.getAssetMap().getAsset(effectId)` also returns nullable. Both are dereferenced unconditionally on L243. If either is null, the particle loop NPEs on every spawn cycle, permanently killing the highlight for that player. The design doc explicitly lists "Guard nullable effect lookup before addEffect" as a required change. |
| 2 | Anti-pattern | 🔴 Blocked | [BlueprintBookParticleLoop.java](src/main/java/com/CodeCreature/ui/blueprintbook/BlueprintBookParticleLoop.java#L133) | **Missing null guard on `getHotbar()`.** Compiler reports "Potential null pointer access: The method getHotbar() may return null." The call `player.getInventory().getHotbar().getItemStack(activeSlot)` dereferences the result directly. If `getHotbar()` returns null, the update loop NPEs on every tick. |
...
| 6 | Anti-pattern | 🟡 Should Fix | [BlueprintBookParticleLoop.java](src/main/java/com/CodeCreature/ui/blueprintbook/BlueprintBookParticleLoop.java#L132-L133), [BlueprintBookParticleLoop.java](src/main/java/com/CodeCreature/ui/blueprintbook/BlueprintBookParticleLoop.java#L163) | **Deprecated-for-removal Inventory API usage.** Same deprecated APIs as finding #5: `getActiveHotbarSlot()`, `getHotbar()`, and `getCombinedBackpackStorageHotbar()`. |
...
| 10 | Scalability | 🔵 Review | [BlueprintBookParticleLoop.java](src/main/java/com/CodeCreature/ui/blueprintbook/BlueprintBookParticleLoop.java#L154) | **Unchecked @Nonnull conversion warning.** `blockType.getId()` returns `@Nullable String` but `getRecipeForBlock()` requires `@Nonnull String`. The null guard at L148 (`blockType == null`) doesn't guard `blockTypeId` itself. Low risk since `getId()` shouldn't return null for a valid BlockType, but the compiler flags it. |
...
1. **Finding #1 + #3 (Blocked):** The particle loop will NPE the first time a player looks at a craftable block while holding the Blueprint Book. The `effectCtrl.addEffect(entityRef, effect, ...)` call on L243 has three nullable arguments passed to `@Nonnull` parameters. This kills the scheduled loop for that player permanently (exception caught, `active` remains true, loop keeps retrying and failing).
...
| File | What to remove |
|------|---------------|
| `BlueprintBookParticleLoop.java` | `COMPILE_RESTORATION_TODO_ID` constant (L48), `compileRestorationSkeleton()` method (L68-L78), 3 TODO comments |
| `BlueprintBookPickStencilInteraction.java` | `COMPILE_RESTORATION_TODO_ID` constant (L35), `compileRestorationSkeleton()` method (L47-L55), 2 TODO comments |
...
| Check | Status |
|-------|--------|
| `Plugin.java` imports `BlueprintBookParticleLoop` from `ui.blueprintbook` | ✅ Correct (L12) |
| `Plugin.java` references `BlueprintBookPickStencilInteraction` from `ui.blueprintbook` | ✅ Correct (L78-L79, FQCN) |
| `Plugin.java` imports `StencilInputListener` from `ui.radial` | ✅ Correct (L15) |
| `ParticleCommand.java` imports from `ui.blueprintbook` | ✅ Correct (L3) |
| Test files in correct package | ✅ Both in `com.CodeCreature.ui.blueprintbook` |
| No stale `com.CodeCreature.stencil.BlueprintBook*` imports in `src/` | ✅ Only in `all_java_sources.txt` (historical) |
...
| `Vector3d` imported from `org.joml` | ✅ `BlueprintBookParticleLoop`, `BoundingBoxRayCast` |
| `Rotation3f` imported from `com.hypixel.hytale.math.vector` | ✅ `BlueprintBookParticleLoop` |
...
| `BlueprintBookPickStencilInteraction` L93 — stencil give log uses `%s` | ✅ `log("[BlueprintBook] Gave stencil for %s to player %s", ...)` |
