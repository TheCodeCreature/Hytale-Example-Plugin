...
id: S2604301421
type: story
title: "Add Blueprint BSON Guard to PlacementCostScaler"
status: backlog
priority: high
feature: F2604301420
epic: E2604301400
...
# Add Blueprint BSON Guard to PlacementCostScaler

## User Story
As a **player**, I want the natural block placement cost scaler to ignore my blueprint stencils so that I am not double-charged when placing a natural block via stencil.

## Acceptance Criteria

### Checklist
- [ ] `PlacementCostScaler.handle()` reads BSON metadata from `event.getItemInHand()`
- [ ] If the metadata contains the blueprint tag, the handler returns immediately
...
- **Given** the player holds a `Rock_Stone` stencil (BSON: `{blueprint: true}`) and `Rock_Stone` is registered as a natural block
