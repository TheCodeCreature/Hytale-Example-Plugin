...
## Description
When holding the Blueprint Book, middle-clicking on a placed block resolves it to a crafting recipe and gives the player the corresponding stencil item — replacing the normal "pick block" creative-mode behavior.
...
- **Given** player holds Blueprint Book, aims at a Hardwood Planks block, recipe is encountered
...
- **Given** player holds Blueprint Book, aims at a Dirt block
- **When** player middle-clicks
- **Then** nothing happens (no stencil, no error)

**Pick unencountered recipe**
- **Given** player holds Blueprint Book, aims at a block whose recipe they haven't encountered
- **When** player middle-clicks
