...
## User Story
As a **player**, I want the **Blueprint Bench to show recipes from both Builders and Furniture benches** so that **I can select any placeable recipe for my building tool**.
...
- [ ] The Blueprint Bench displays all mutated recipes in its categories
...
- **Given** a Builders Bench and Blueprint Bench both exist
- **When** the player opens each
- **Then** the Builders Bench shows only its original recipes; the Blueprint Bench shows both
...
- KEY CONSIDERATION: Furniture recipes are `BenchType: "Crafting"`, but the Blueprint Bench is `"StructuralCrafting"`. The mutated requirement must match the Blueprint Bench's type.
