...
| 15 | DELETE | 🔴 Blocked | [PlaceholderRow.ui](../src/main/resources/Common/UI/Custom/Pages/StencilBook/PlaceholderRow.ui) | UI component for placeholder list rows. Appended by `StencilSelectionPage.build()`. |
| 16 | DELETE | 🔴 Blocked | [PlaceholderSlot.ui](../src/main/resources/Common/UI/Custom/Pages/StencilBook/PlaceholderSlot.ui) | UI component for placeholder slot interaction. |
...
| 23 | KEEP | ✅ Safe | [PlaceBlockCostUtil.java](../src/main/java/com/UnobstructedThirdPerson/placeblock/PlaceBlockCostUtil.java) | Shared by `StencilPlacementSystem`, `StencilSelectionPage`, and (deleted) `PlaceholderSyncSystem`/`PlaceBlockToolInteraction`. After removal, still has 2 active consumers. |
| 24 | KEEP | ✅ Safe | [StencilBookRecipeMutator.java](../src/main/java/com/UnobstructedThirdPerson/placeblock/StencilBookRecipeMutator.java) | Creates shadow recipes with `PlaceBlock` ResourceType input and `Stencil` bench requirement. The input override comment (L85) mentions "matches all 3 placeholder colors" — **update comment only**. The `ResourceType` input still works for stencils. |
...
- `src/main/resources/Common/UI/Custom/Pages/StencilBook/PlaceholderRow.ui`
- `src/main/resources/Common/UI/Custom/Pages/StencilBook/PlaceholderSlot.ui`

### Files to EDIT (5 files)

1. **`UnobstructedThirdPersonPlugin.java`** — Remove 5 imports, 5 registration/lifecycle calls
2. **`StencilSelectionPage.java`** — Remove 2 imports, 1 constant, ~120 lines of placeholder methods/handlers
3. **`StencilBookPage.ui`** — Remove ~38 lines (Placeholder List section)
...
| `StencilSelectionPage.java` | L3 | `import BlockPreviewReskinManager` |
| `StencilSelectionPage.java` | L5 | `import PlaceBlockMetadata` |
...
| **`StencilBookRecipeMutator` shadow recipes still use `PlaceBlock` ResourceType** | The input override `new MaterialQuantity(null, "PlaceBlock", null, 1, null)` was designed for placeholder colors. Stencils also match this ResourceType. | **Verify stencil items have the `PlaceBlock` ResourceType** — if not, recipe matching will break. |
...
- [ ] `StencilSelectionPage` opens, shows recipes, handles stencil workflow
...
- [ ] `StencilBookRecipeMutator.mutate()` still runs on `LoadAssetEvent`

---

→ @Engineer implement migration from docs/review-placeholder-removal-map.md  
→ @Architect if the `PlaceBlock` ResourceType input in `StencilBookRecipeMutator` needs redesign for stencil-only workflows
