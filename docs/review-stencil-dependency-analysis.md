# Stencil System Dependency Analysis

## Executive Summary

The codebase contains **~100 source files** across 15 packages. The **stencil system** (Stencil Book, Stencil Crafting, Stencil, PlaceBlock UI) accounts for **~50 files** — roughly half the codebase. The remaining **~50 files** belong to four independent systems: Camera Transparency, Shape Engine, Drop Scaling/Resource Economy, Portable Bench, Movement, Preview, and Debug utilities. The critical boundary is the `resourcecollection/` package, which is **shared infrastructure** — 7 files are required by the stencil system, while 11 files serve only the drop-scaling pipeline.
...
graph TB
    ...
    subgraph STENCIL_SYSTEM["STENCIL SYSTEM"]
        direction TB
        subgraph STENCIL["Stencil Package"]
            ...
            S8["StencilBookParticleLoop"]
            S9["StencilBookPickStencilInteraction"]
            S10["RadialSegmentItem"]
        end

        subgraph PLACEBLOCK["PlaceBlock Package"]
            PB1["StencilBookRecipeMutator"]
        ...
        subgraph PLACEBLOCK_UI["PlaceBlock UI"]
            UI1["StencilSelectionPage"]
            UI2["StencilBookOpenUIInteraction"]
            UI3["StencilBookPrefs"]
            UI4["StencilBookPrefsStore"]
        ...
        subgraph STENCIL_CMD["Stencil Commands"]
        ...
        subgraph STENCIL_UTIL["Stencil Utils"]
            BU1["ShapeAwareRaycast"]
            BU2["StencilBookQuickSelectPage"]
        end
    end

    subgraph NOT_STENCIL["NOT-STENCIL SYSTEMS"]
        direction TB
    ...
    style STENCIL_SYSTEM fill:#d4edda,stroke:#28a745
    style NOT_STENCIL fill:#f8d7da,stroke:#dc3545
    style PLUGIN fill:#cce5ff,stroke:#004085
```

---

## STENCIL — Files to KEEP (50 files)
...
| 8 | `stencil/StencilBookParticleLoop.java` | `BenchRecipeRegistries`, `RecipeAffordabilityResolver`, `ShapeAwareRaycast` | Particle effects on targeted block while holding stencil book |
| 9 | `stencil/StencilBookPickStencilInteraction.java` | `BenchRecipeRegistries`, `FilteredRecipeEntry`, `RecipeFilterRegistry`, `StencilMetadata` | Interaction: raycast to block → create stencil from its recipe |
...
| # | File | Key Dependencies (internal) | Reason |
|---|------|---------------------------|--------|
| 11 | `placeblock/StencilBookRecipeMutator.java` | _(Hytale API only)_ | Creates shadow Stencil_ recipes at asset load time |
...
| # | File | Key Dependencies (internal) | Reason |
|---|------|---------------------------|--------|
| 17 | `placeblock/ui/StencilSelectionPage.java` | `PlaceBlockCostUtil`, `RecipeAffordabilityResolver`, `ResolvedIngredient`, `StencilMetadata`, `BenchCategory`, `FilteredRecipeEntry`, `RecipeFilterRegistry`, `IngredientTree*`, `RecipeFilterPipeline` | Main Stencil Crafting UI page — recipe grid, filters, detail panel |
| 18 | `placeblock/ui/StencilBookOpenUIInteraction.java` | `StencilSelectionPage` | Interaction type that opens the Stencil Crafting custom page |
| 19 | `placeblock/ui/StencilBookPrefs.java` | `AffordabilityMode` | Serializable preferences for bench UI state |
| 20 | `placeblock/ui/StencilBookPrefsStore.java` | `StencilBookPrefs` | File-based persistence for per-player bench preferences |
...
### `resourcecollection/` — Shared Dependencies Used by Stencil (7 files)

These files are **shared infrastructure** used by both the stencil system and the drop-scaling system. The stencil system directly imports them.

| # | File | Stencil Consumers | Reason |
|---|------|-------------------|--------|
| 35 | `resourcecollection/RecipeFilterRegistry.java` | `StencilBookPickStencilInteraction`, `StencilRadialMenuPage`, `StencilSelectionPage` | Single source of truth for validated, filtered recipes |
| 36 | `resourcecollection/FilteredRecipeEntry.java` | `StencilBookPickStencilInteraction`, `StencilRadialMenuPage`, `StencilSelectionPage` | Immutable recipe snapshot data record |
| 37 | `resourcecollection/BenchCategory.java` | `RecipeAffordabilityResolver`, `StencilRadialMenuPage`, `StencilSelectionPage` | Bench classification enum controlling natural/non-natural preference |
| 38 | `resourcecollection/BenchRecipeRegistries.java` | `StencilBookParticleLoop`, `StencilBookPickStencilInteraction` | Aggregate recipe lookup across all bench registries |
...
### `command/placeblock/` + `command/ParticleCommand` — Stencil Commands (4 files)

| # | File | Key Dependencies (internal) | Reason |
|---|------|---------------------------|--------|
| 42 | `command/placeblock/PlaceBlockCommand.java` | `GridTestSubCommand`, `StencilSubCommand` | Parent command for stencil tool commands |
...
| 45 | `command/ParticleCommand.java` | `StencilBookPickStencilInteraction` | Gets/sets stencil book particle effect |

### `stencilbook/` + `util/` — Stencil Utilities (2 files)

| # | File | Key Dependencies (internal) | Reason |
|---|------|---------------------------|--------|
| 46 | `stencilbook/StencilBookQuickSelectPage.java` | _(empty file)_ | Placeholder for stencil book quick-select page |
| 47 | `util/ShapeAwareRaycast.java` | _(Hytale API only)_ | Hitbox-accurate raycast; used by `StencilBookParticleLoop` |

### Stencil Test Files (3 files)
...
## NOT-STENCIL — Files to REMOVE (51 files + 1 entry point)

### Plugin Entry Point (1 file — SHARED, do not remove)

| # | File | Reason |
|---|------|--------|
| — | `UnobstructedThirdPersonPlugin.java` | **SHARED** — registers both stencil and non-stencil systems. Must be refactored to remove non-stencil registrations, not deleted. |

### `camera/` — Camera Transparency System (3 files)

| # | File | Key Dependencies | Reason |
|---|------|-----------------|--------|
| 1 | `camera/CameraTransparencyVolume.java` | `shape/v1/*`, `BlockSnapshot` | V1 camera transparency volume — no stencil imports |
| 2 | `camera/v2/CameraTransparencyVolumeV2.java` | `shape/v2/*`, `BlockSnapshot`, `DebugCube` | V2 camera transparency volume — no stencil imports |
...
| # | File | Reason |
|---|------|--------|
| 36 | `resourcecollection/DropScaler.java` | Main drop scaling entry point — no stencil code imports this |
...
| # | File | Reason |
|---|------|--------|
| 53 | `movement/NewMovementSystem.java` | Moon gravity system — no stencil imports |
| 54 | `command/NewMovementCommand.java` | Movement command — no stencil imports |
| 55 | `preview/PreviewBlockManager.java` | Preview/ghost blocks — no stencil imports |
| 56 | `command/PreviewCommand.java` | Preview sphere command — no stencil imports |
| 57 | `depricated/CameraPositionUtil.java` | Deprecated camera util — no stencil imports |
| 58 | `records/BlockSnapshot.java` | Block data record — used by camera/preview only, not by stencil code |
...
### Non-Stencil Test Files (17 files)
...
These are the import edges that cross the stencil/not-stencil boundary:

| From (NOT-STENCIL) | To (STENCIL) | Import Direction | Risk |
|---|---|---|---|
| `PlacementCostScaler` | `StencilMetadata` | NOT-STENCIL → STENCIL | **One-way check** — PlacementCostScaler imports StencilMetadata to SKIP stencils during natural block cost enforcement. If stencil system is extracted to a separate module, this import breaks. |
| `DropScaler.apply()` | `BenchRecipeRegistries.init()`, `RecipeFilterRegistry.init()` | NOT-STENCIL → SHARED | DropScaler initializes the shared registries during `LoadAssetEvent`. If drop scaling is removed, the stencil system needs its own initialization path for these registries. |
| `UnobstructedThirdPersonPlugin` | Both systems | SHARED → BOTH | Plugin entry point registers everything. Must be refactored to separate stencil and non-stencil registrations. |
...
2. **`PlacementCostScaler` has a reverse dependency** — it imports `StencilMetadata` (stencil) to detect and skip stencils. If separating into modules, this needs an interface or the stencil-detection logic needs to be inverted (e.g., tag-based check without importing `StencilMetadata` directly).

3. **`BlockSnapshot` is NOT used by stencil code** — despite being in `records/`, it's only consumed by camera, preview, and debug commands. Stencil code uses `CraftingRecipe`, `MaterialQuantity`, and `ItemStack` from the Hytale API directly.

4. **The `assignbench/` directory is empty** — can be deleted.

5. **`StencilBookQuickSelectPage.java` is an empty file** — placeholder only.
