# Review: StencilBookParticleLoop Performance

**File:** [src/main/java/com/UnobstructedThirdPerson/stencil/StencilBookParticleLoop.java](../src/main/java/com/UnobstructedThirdPerson/stencil/StencilBookParticleLoop.java)
**Date:** 2026-05-08
**Trigger:** User-reported significant lag while holding Stencil Book
...
|---|----------|----------|----------|--------|
| 1 | Anti-pattern | 🔴 Critical | [StencilBookParticleLoop.java](../src/main/java/com/UnobstructedThirdPerson/stencil/StencilBookParticleLoop.java#L110) | **Unconditional server-side raycast every 100ms.** `TargetUtil.getTargetBlock()` runs a full voxel raycast (up to 8 blocks) every tick even when the player's position and head rotation haven't changed. With N players holding books, this is N×10 raycasts/second on the world thread. **Fix:** Cache the player's position + head rotation. Skip the raycast if neither changed since last tick. |
...
| 8 | Redundancy | 🔵 Low | [StencilBookParticleLoop.java](../src/main/java/com/UnobstructedThirdPerson/stencil/StencilBookParticleLoop.java#L139) | **`getCombinedBackpackStorageHotbar()` is NOT an allocation concern.** Verified: `Inventory.java` line 557 returns a cached `combinedBackpackStorageHotbar` field — no object created per call. This is a non-issue. |
...
5. **Add volatile active flag for early-out before world.execute() (Finding #5)**
   ...
   - Set false when held item isn't StencilBook
...
```
→ @Engineer implement migration from docs/review-stencil-book-particle-loop.md
