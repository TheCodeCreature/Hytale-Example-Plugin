...
| F6 | Excessive logging | **HIGH** | [BlueprintSelectionPage.java](src/main/java/com/CodeCreature/ui/bench/BlueprintSelectionPage.java#L143-L193) | **10 `INFO`-level log statements fire on every menu open** in `loadRecipes()` and `buildCategoryInfoMap()`. Several perform string concatenation with collections (L181: `tabKeyCounts`, L183: `benchIds`, L193: `recipeCatIds`, L850: `map.keySet()`). The per-recipe log inside the `for` loop (L158-161) fires for every recipe where a bench ID resolves differently — potentially hundreds of times. These all run on the server thread, forcing string building + I/O. |
| F7 | Excessive logging | **MEDIUM** | [BlueprintSelectionPage.java](src/main/java/com/CodeCreature/ui/bench/BlueprintSelectionPage.java#L826-L850) | **`buildCategoryInfoMap()` logs every top-level and child category at INFO.** Lines 830-845 log per-category including children. With ~12 top-level + ~40 child categories, that's ~50+ INFO log calls on every open, each doing string concatenation. |
| F8 | Excessive cmd.set() volume | **HIGH** | [GridLayoutController.java](src/main/java/com/CodeCreature/ui/bench/GridLayoutController.java#L65-L120) + [BlueprintSelectionPage.java](src/main/java/com/CodeCreature/ui/bench/BlueprintSelectionPage.java#L371-L375) | **Grid update writes ~2000+ `cmd.set()` commands per update.** For 500 recipes across ~15 sets: each visible cell = 5 `cmd.set()` calls (Visible, ItemId, Dim.Visible, Btn.Style, + group header). Each hidden cell = 2 calls. Hidden groups = 1 call each. Total estimate for 500 cells: ~2500 `cmd.set()` calls for the grid alone. Add set filters (~100), material groups (~100), bench tabs (~50), detail panel (~40), ingredient tree (~200+). A single `sendUpdate` could carry **3000+ commands**. |
| F9 | Disk I/O on server thread | **HIGH** | [BlueprintBookPrefsStore.java](src/main/java/com/CodeCreature/ui/bench/BlueprintBookPrefsStore.java#L44-L53) | **`savePrefs()` writes BSON to disk synchronously on the server thread.** Called on every tab switch, every filter toggle, every search input, every recipe select — 8 call sites in `handleDataEvent()`. Each call does BSON encode + file write. Under rapid interaction, this is N synchronous disk writes blocking the server thread. |
| F10 | Debouncing gaps | **CRITICAL** | [BlueprintSelectionPage.java](src/main/java/com/CodeCreature/ui/bench/BlueprintSelectionPage.java#L551-L560) | **Search input has no debounce.** Every keystroke fires `handleDataEvent()` with `data.searchQuery`, which runs `applyFilter()` (500+ recipe pipeline), `pruneInvalidMaterialGroups()` (possible second pipeline run), full grid UI update (~2500 cmd.set), `savePrefs()` (disk write), and `sendUpdate()` (network packet). Typing "wood" = 4 full pipeline runs + 4 disk writes + 4 network packets, all synchronous on the server thread. |
| F11 | Redundant computation | **MEDIUM** | [BlueprintSelectionPage.java](src/main/java/com/CodeCreature/ui/bench/BlueprintSelectionPage.java#L280-L284) | **Prefs loaded twice during `build()`.** `BlueprintBookPrefsStore.load()` is called at L280 and again at L365 (to restore ingredient selections). Each call reads from disk and decodes BSON. |
| F12 | Quadratic scan | **MEDIUM** | [BlueprintSelectionPage.java](src/main/java/com/CodeCreature/ui/bench/BlueprintSelectionPage.java#L800-L811) + [DetailPanelController.java](src/main/java/com/CodeCreature/ui/bench/DetailPanelController.java#L125-L129) | **`findEntry()` is an O(N) linear scan** over ~500 recipes. Called from `DetailPanelController.updateUI()` on every interaction that triggers `updateDetail()`. Also called in `giveSelectedBlueprint()`. With 500 recipes, each scan touches up to 500 records. Not quadratic alone, but it's called inside flows that already do O(N) work. |
| F13 | Missing caching | **HIGH** | [BlueprintSelectionPage.java](src/main/java/com/CodeCreature/ui/bench/BlueprintSelectionPage.java#L136-L206) | **`loadRecipes()` rebuilds everything from scratch on every open.** Recipe data, bench IDs, category info map, ingredient tree — all recomputed from the registry on every `build()` call. The recipe registry is static at runtime. This is the dominant cost of the initial open. |
| F14 | Object churn | **MEDIUM** | [BlueprintSelectionPage.java](src/main/java/com/CodeCreature/ui/bench/BlueprintSelectionPage.java#L95-L130) | **`computeMaxLayout()` creates a full `InputRecipe` list then walks the entire pipeline output.** The `MaxLayoutInfo` never changes between opens (it depends only on the static recipe registry). It could be computed once and cached. |
