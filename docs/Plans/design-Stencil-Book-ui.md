...
> **Supersedes:** Inline `appendInline()` UI in current `StencilSelectionPage.java`

## 1. Overview

Replace the current inline-markup UI in `StencilSelectionPage` with two custom `.ui` template files: a main page layout (`StencilBookPage.ui`) and a reusable recipe list entry (`RecipeEntry.ui`). The page lets players browse block-producing recipes, select one, and arm their held `Block_Placeholder` item with the selected recipe. This separates layout/styling concerns from Java server logic and makes the UI maintainable.
...
classDiagram
    ...
    class StencilBookOpenUIInteraction {
        +firstRun(type, context, cooldown) void
    }

    StencilSelectionPage --> StencilBookPageUI : loads via append
    StencilSelectionPage --> RecipeEntryUI : appends N items
    StencilSelectionPage --> EventPayload : deserializes events
    StencilBookOpenUIInteraction --> StencilSelectionPage : creates
```
...
graph TB
    subgraph "Client (.ui files)"
        PAGE_UI["StencilBookPage.ui<br/>Page structure"]
        ENTRY_UI["RecipeEntry.ui<br/>List item template"]
        COMMON["Common.ui<br/>Shared styles"]
    end

    subgraph "Server (Java)"
        INTERACTION["StencilBookOpenUIInteraction<br/>Block right-click handler"]
        PAGE["StencilSelectionPage<br/>Page logic + state"]
...
sequenceDiagram
    ...
    participant INT as StencilBookOpenUIInteraction
    participant PAGE as StencilSelectionPage
    participant CLIENT as Client UI

    P->>INT: Right-click Stencil Crafting
    INT->>PAGE: new StencilSelectionPage(playerRef)
    INT->>PAGE: openCustomPage(ref, store, page)
    PAGE->>PAGE: build(ref, cmd, evt, store)
    PAGE->>CLIENT: cmd.append("Pages/StencilBook/StencilBookPage.ui")
    loop Each visible recipe
        PAGE->>CLIENT: cmd.append("#RecipeList", "Pages/StencilBook/RecipeEntry.ui")
...
### 7.1 `StencilBookPage.ui`

**Path:** `src/main/resources/Common/UI/Custom/Pages/StencilBook/StencilBookPage.ui`
**Loaded via:** `commandBuilder.append("Pages/StencilBook/StencilBookPage.ui")`
...
**Path:** `src/main/resources/Common/UI/Custom/Pages/StencilBook/RecipeEntry.ui`
**Loaded via:** `commandBuilder.append("#RecipeList", "Pages/StencilBook/RecipeEntry.ui")`
...
Replace `"Pages/BasicTextButton.ui"` with `"Pages/StencilBook/RecipeEntry.ui"` and use `#RecipeName.Text` instead of `.TextSpans`:
    ...
    for (int i = 0; i < showing; i++) {
        ...
        if (entry.recipeId().equals(this.selectedRecipeId)) {
            cmd.set("#RecipeList[" + i + "].Style",
                    Value.ref("Pages/StencilBook/RecipeEntry.ui", "SelectedStyle"));
        }
    ...
    if (selectedRecipeId != null) {
        ...
        if (entry != null) {
            ...
            cmd.set("#ConfirmButton.Style",
                    Value.ref("Pages/StencilBook/StencilBookPage.ui", "ConfirmButtonStyle"));
        }
    } else {
        ...
        cmd.set("#ConfirmButton.Style",
                Value.ref("Pages/StencilBook/StencilBookPage.ui", "DisabledConfirmStyle"));
    }
}
```
...
|------|-----------------|----------|
| `StencilBookPage.ui` | `@ConfirmButtonStyle` | Enabled confirm button |
| `StencilBookPage.ui` | `@DisabledConfirmStyle` | Disabled confirm button (no selection) |
...
└── Pages/
    └── StencilBook/
        ├── StencilBookPage.ui          ← Main page layout
        └── RecipeEntry.ui                 ← Recipe list item template

src/main/java/com/UnobstructedThirdPerson/placeblock/ui/
├── StencilBookOpenUIInteraction.java   ← No changes
└── StencilSelectionPage.java            ← Modified: use .ui files instead of appendInline()
```
...
| File | Change | Detail |
|------|--------|--------|
| `StencilSelectionPage.java` | **Modify `build()`** | Replace `appendInline()` call with `cmd.append("Pages/StencilBook/StencilBookPage.ui")`. Replace `#SelectButton` event binding with `#ConfirmButton`. |
| `StencilSelectionPage.java` | **Modify `buildRecipeList()`** | Replace `"Pages/BasicTextButton.ui"` with `"Pages/StencilBook/RecipeEntry.ui"`. Use `#RecipeName.Text` instead of `.TextSpans`. Use `Value.ref("Pages/StencilBook/RecipeEntry.ui", "SelectedStyle")` for selection highlight. |
| `StencilSelectionPage.java` | **Rename `updateFooter()` → `updateDetailPanel()`** | Expand to set `#OutputName.Text`, `#CostSummary.Text`, and toggle `#ConfirmButton.Style` between enabled/disabled styles. |
| `StencilSelectionPage.java` | **Modify `handleDataEvent()`** | Change `"Select".equals(data.action)` to `"Confirm".equals(data.action)`. Call `updateDetailPanel()` instead of `updateFooter()`. |
| `StencilSelectionPage.java` | **Remove `BUTTON_STYLE` / `BUTTON_STYLE_SELECTED` constants** | These referenced `Pages/BasicTextButton.ui` — no longer needed. |
| `StencilBookOpenUIInteraction.java` | **No changes** | Already creates `StencilSelectionPage` and opens it correctly. |
| `manifest.json` | **No changes** | Already has `"IncludesAssetPack": true`. |

### What can be deleted after migration

- The `BUTTON_STYLE` and `BUTTON_STYLE_SELECTED` static fields in `StencilSelectionPage.java`
...
**Fix applied to `StencilBookOpenUIInteraction.java`:**
...
- The Confirm-time check in `StencilSelectionPage.handleDataEvent()` remains as a safety net
...
2. **`StencilSelectionPage.java`** — New affordability check in `buildRecipeList()`:
   ...
   - If unaffordable: `cmd.set("#RecipeList[i].Style", Value.ref("Pages/StencilBook/RecipeEntry.ui", "UnaffordableStyle"))`
...
4. **Page constructor change:** `StencilSelectionPage` now accepts `Ref<EntityStore>` and `Store<EntityStore>` in addition to `PlayerRef`, so it can access inventory during `loadRecipes()` and `buildRecipeList()`.
...
- [x] `.ui` file contents specified (StencilBookPage.ui + RecipeEntry.ui)
