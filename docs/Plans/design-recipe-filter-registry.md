...
RecipeFilterRegistry is a shared, read-only registry that scans all `CraftingRecipe` assets exactly once, applies common validation and bench-matching predicates, and stores the results as indexed `FilteredRecipeEntry` records. It eliminates the duplicated recipe scanning logic in `BlueprintSelectionPage` (UI) and `BenchRecipeRegistry` (DropScaler pipeline), fixes the single-BenchRequirement bug in the UI code, and establishes `BenchCategory` as the single source of truth for allowed bench IDs.
...
2. **Correctness** — BenchRequirement matching scans ALL entries (fixes BlueprintSelectionPage bug)
...
classDiagram
    ...
    BlueprintSelectionPage ..> ResourceTypeResolver : resolves inputs
...
graph TB
    ...
    subgraph UIConsumer["BlueprintSelectionPage"]
...
├── placeblock/
│   └── ui/
│       └── BlueprintSelectionPage.java ← MODIFIED: reads from RecipeFilterRegistry
```
...
- `BlueprintSelectionPage` already imports from `resourcecollection` indirectly via the crafting API
...
```java
NaturalResourceRegistry.init();
RecipeFilterRegistry.init(Set.of("Blueprint_", "Salvage"));
...
### 7.5 BlueprintSelectionPage — replace loadRecipes() scanning
...
| File | What to remove |
|------|---------------|
| `BlueprintSelectionPage.java` | `ALLOWED_BENCHES` field, `ITEM_SET_FIELD` field + static block, `resolveIngredientItemId()` method, recipe scanning loop in `loadRecipes()` |
...
2. **Skip-prefix extensibility**: The default set `{"Blueprint_", "Salvage"}` covers both current consumers. Should additional prefixes be configurable at runtime, or is compile-time sufficient? Current design uses `Set<String>` parameter on `init()` which is flexible enough.

3. **Input validation predicate**: `BlueprintSelectionPage` requires at least one input with `itemId` or `resourceTypeId`. `BenchRecipeRegistry` does not check this explicitly (recipes without valid inputs would just produce empty cost data). Should the shared registry enforce this? Recommendation: yes — a recipe with no resolvable inputs is useless to both consumers.
