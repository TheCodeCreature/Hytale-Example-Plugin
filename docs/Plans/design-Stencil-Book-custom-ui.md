...
graph TB
    ...
    subgraph "Phase B: Custom UI"
        ...
        INTERCEPT --> PAGE["StencilSelectionPage opened"]
...
sequenceDiagram
    ...
    P->>CMD: /placeblock assign Stencil_Wall_Stone_Cobble
...
- Accepts both shadow IDs (`Stencil_Wall_Stone_Cobble`) and original IDs (`Wall_Stone_Cobble`) — if original ID given, prepend `Stencil_` and look up shadow
...
- Lists all shadow recipe IDs (from `StencilBookRecipeMutator`), optionally filtered by bench category
...
### 4.2 Page Design: `StencilSelectionPage`

```java
public class StencilSelectionPage 
    extends InteractiveCustomUIPage<StencilSelectionPage.SelectionEventData> {
...
Register a new interaction type (like `PortableBenchInteraction`) on the Stencil Crafting block's item/interaction codec. When the player right-clicks the bench while holding a placeholder, open `StencilSelectionPage` instead of the standard bench.
...
The recipe list comes from `StencilBookRecipeMutator`'s shadow recipes. At page construction:
1. Iterate `CraftingRecipe.getAssetMap()` for recipes with `Stencil` bench requirement
...
| **`PlaceBlockMenuInteraction`** | **KEEP — repurpose in Phase B** | Currently a stub. In Phase B, this interaction can be the trigger that opens `StencilSelectionPage` when the player right-clicks the bench while holding a placeholder. Already registered as `"PlaceBlock_Menu"` codec. |
| **`StencilBookRecipeMutator`** | **KEEP — role changes** | Shadow recipes still needed as the canonical list of "what can be built." Phase B uses them to populate the selection page. The shadow recipes' `BenchRequirement` with `Stencil` ID is still useful for filtering, even if no `StructuralCraftingWindow` reads them. |
...
| `Bench.Id` | `"Stencil"` | Keep | May become irrelevant if no `StructuralCraftingWindow` is involved |
| Shadow recipes | Exist with `BenchRequirement.Id = "Stencil"` | Keep — used for recipe lookups | Keep — used to populate selection page |
...
**Phase B:** Additionally register the `StencilSelectionPage` opening logic (either via a modified `PlaceBlockMenuInteraction` or a new block interaction handler).
...
3. **Open `StencilSelectionPage`** from the interaction handler
...
1. Create `StencilSelectionPage` extending `InteractiveCustomUIPage`
