...
classDiagram
    ...
    StencilSelectionPage --> StencilBookStyles_ui : Value.ref styles
    StencilSelectionPage --> RecipeIconCell_ui : cmd.set cell state
    StencilSelectionPage --> CostCell_ui : cmd.set cost state
    StencilSelectionPage --> StencilBookPage_ui : cmd.set output state
```
...
├── StencilBookPage.ui        ← MODIFIED (add #OutputFrame, #OutputDim)
├── StencilBookStyles.ui      ← MODIFIED (add 3 new styles)
...
└── StencilSelectionPage.java  ← MODIFIED (new constants, updated methods, new helper)
```
...
### 7.3 StencilBookPage.ui — Add `#OutputFrame` ID and `#OutputDim` overlay
...
Add to `StencilSelectionPage.java` alongside existing `FILTER_ACTIVE`/`FILTER_INACTIVE`:

```java
// Story 1: Selected cell highlight
private static final Value<String> CELL_SELECTED_STYLE =
    Value.ref("Pages/StencilBook/StencilBookStyles.ui", "SelectedCellButtonStyle");
private static final Value<String> CELL_UNSELECTED_STYLE =
    Value.ref("Pages/StencilBook/StencilBookStyles.ui", "TransparentButtonStyle");
...
private static final Value<String> DETAIL_LABEL_NORMAL =
    Value.ref("Pages/StencilBook/StencilBookStyles.ui", "DetailLabelStyle");
private static final Value<String> DETAIL_LABEL_MUTED =
    Value.ref("Pages/StencilBook/StencilBookStyles.ui", "DetailLabelMutedStyle");
...
| File | Change | Details |
|------|--------|---------|
| `StencilBookStyles.ui` | Add 3 style definitions | `@SelectedCellButtonStyle`, `@CostQuantityInsufficientStyle`, `@DetailLabelMutedStyle` |
| `CostCell.ui` | Add `#CostDim` group | Dim overlay between `#CostIcon` and `#CostQty` — matches `RecipeIconCell.ui` pattern |
| `StencilBookPage.ui` | Add `#OutputFrame` ID, `#OutputDim` group | ID on existing output background group; dim overlay after `#OutputIcon` |
| `StencilSelectionPage.java` | Add 8 constants | 6 `Value.ref` + 2 string background paths |
| `StencilSelectionPage.java` | Add `countItemInInventory()` | New private helper method |
| `StencilSelectionPage.java` | Modify `updateRecipeGrid()` | Add `#CellBtn.Style` set per cell |
| `StencilSelectionPage.java` | Modify `hideRemainingCells()` | Reset `#CellBtn.Style` on hidden cells |
| `StencilSelectionPage.java` | Modify `updateDetailPanel()` | Add per-ingredient and output-level affordability logic |
...
#### Unit: StencilBookStyles.ui — New style definitions
...
#### Unit: StencilBookPage.ui — Add output frame IDs and dim overlay
...
- **Changes**: Add `countItemInInventory(CombinedItemContainer, String)` to `StencilSelectionPage.java`
...
- **Dependencies**: Wave 1 `StencilBookStyles.ui` (style names must match)
...
- **Dependencies**: Wave 1 `CostCell.ui`, `StencilBookPage.ui`, `countItemInInventory()`; Wave 2 Java constants
...
→ @Engineer implement docs/design-Stencil-Book-visual-states.md
